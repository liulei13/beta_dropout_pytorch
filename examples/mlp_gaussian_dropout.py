...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc3 = nn.Sequential(
            nn.Linear(800, 10) )
        self.dropout_layer1 = Gaussian_Dropout(args.mu, args.sigma2, self.training, False)
        self.dropout_layer2 = Gaussian_Dropout(args.mu, args.sigma2, self.training, False)
...
#ptimizer = optim.Adam(model.parameters(),lr=args.lr,weight_decay=1e-8)
...
def train(epoch):
    model.train()
    model.dropout_layer1 = Gaussian_Dropout(args.mu, args.sigma2, model.training, False)
    model.dropout_layer2 = Gaussian_Dropout(args.mu, args.sigma2, model.training, False)
...
if __name__=='__main__':
    # mark start time
    t_start=time.time()
    save_dir='results_gaussian_mu0.5'
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    current_filename = os.path.basename(__file__)[:-3]
    log = open(os.path.join(save_dir,current_filename+'_mu_'+str(args.mu)+'_sigma2_'+str(args.sigma2)+'_lr_'+str(args.lr)+'_'+
    ...
    for epoch in range(args.epochs):  # args.epochs
        ...
        if np.min(error_list)<min_error:
            ...
            torch.save(model,
                       os.path.join(save_dir, current_filename + '_mu_' + str(args.mu) + '_sigma2_' + str(args.sigma2) +
