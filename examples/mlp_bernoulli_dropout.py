...
class Net(nn.Module):
    def __init__(self):
        ...
        self.dropout_layer0=nn.Dropout(0.2)
        ...
        self.fc3 = nn.Sequential(
            nn.Linear(800, 10) )
        self.dropout_layer1= nn.Dropout(args.p)
        self.dropout_layer2 =nn.Dropout(args.p)
    ...
    def forward(self, input):
        output = self.dropout_layer0(input.view(-1,28*28))
...
if __name__=='__main__':
    # mark start time
    t_start=time.time()
    save_dir='results_bernoulli'
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    current_filename = os.path.basename(__file__)[:-3]
    log = open(os.path.join(save_dir,current_filename+'_p_'+str(args.p)+'_'+
    ...
    for epoch in range(args.epochs):  # args.epochs
        ...
        if np.min(error_list)<min_error:
            ...
            torch.save(model, os.path.join(save_dir, current_filename + '_p_' + str(args.p) + '_' + str(
                min_error) + '.pth'))
