...
if __name__=='__main__':
    # mark start time
    t_start=time.time()
    save_dir='results_beta_mu0.5'
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    current_filename = os.path.basename(__file__)[:-3]
    log = open(os.path.join(save_dir,current_filename+'_B_'+str(args.B)+'_lr_'+str(args.lr)+'_'+
    ...
    torch.save(model,os.path.join(save_dir,current_filename+'_B_'+str(args.B)+
                                  '_lr_'+str(args.lr)+'_'+str(error_list_npy[0])+'.pth'))
