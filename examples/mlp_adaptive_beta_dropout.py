...
parser.add_argument('--A', type=float, default=0.001, metavar='M',
                    help='A for beta (default:0.5)')
parser.add_argument('--B', type=float, default=0.001, metavar='M',
                    help='B for beta  (default:0.5)')
parser.add_argument('--B_initial', type=float, default=0.5, metavar='M',
                    help='B_initial for beta  (default:0.5)')
parser.add_argument('--anneal_rate', type=float, default=0.001, metavar='M',
                    help='anneal_rate for beta (default:0.001)')
parser.add_argument('--b_max', type=float, default=100.0, metavar='M',
                    help='anneal_rate for beta (default:0.001)')
parser.add_argument('--tp', type=int, default=1500, metavar='M',
                    help='tp for beta (default:0.001)')
...
def train(epoch):
    model.train()
    model.dropout_layer1 = Beta_Dropout(args.B, args.B, model.training, False)
    model.dropout_layer2 = Beta_Dropout(args.B, args.B, model.training, False)
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        optimizer.step()
    if epoch<args.tp:
        model.dropout_layer1.B = epoch*(1.0-args.B_initial)/args.tp+args.B_initial
        model.dropout_layer1.A=model.dropout_layer1.B
        model.dropout_layer2.B = epoch*(1.0-args.B_initial)/args.tp+args.B_initial
        model.dropout_layer2.A = model.dropout_layer2.B
    else:
        model.dropout_layer1.B = (epoch-args.tp)/(args.epochs-args.tp)*(args.b_max-1)+1
        model.dropout_layer1.A = model.dropout_layer1.B
        model.dropout_layer2.B =(epoch-args.tp)/(args.epochs-args.tp)*(args.b_max-1)+1
        model.dropout_layer2.A = model.dropout_layer2.B
    print('dropout_layer1: A:{}  B:{} dropout_layer2: A:{}  B:{}'.format(model.dropout_layer1.A,model.dropout_layer1.B,
                                                                         model.dropout_layer2.A,model.dropout_layer2.B))
...
if __name__=='__main__':
    # mark start time
    t_start=time.time()
    save_dir='results_adaptive_beta_dropout'
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    current_filename = os.path.basename(__file__)[:-3]
    args.B=args.B_initial
    log = open(os.path.join(save_dir,current_filename+'_B_'+str(args.B)+'_lr_'+str(args.lr)+
                            '_'+'to_'+str(args.tp)+'_'+
    ...
    for epoch in range(args.epochs):  # args.epochs
        ...
        if np.min(error_list)<min_error:
            ...
            torch.save(model, os.path.join(save_dir, current_filename + '_B_' + str(args.B) +
                                           '_lr_' + str(args.lr) + '_' + 'tp_' + str(args.tp) + '_' + str(
                min_error) + '.pth'))
    ...
    error_list_npy.sort()
    print_log('Minimal error top 10:',log)
    for i in range(10):
        print_log(str(error_list_npy[i]), log)
